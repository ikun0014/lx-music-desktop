...
export default {
  ...
  getLyric(songInfo) {
    ...
    const promise = (async() => {
      // 优先用 albumAudioId(MixSongID) 精确搜词；旧数据无此字段时回退 songmid
      const result = await this.searchLyric(controller, keyword, duration, songInfo.hash, songInfo.albumAudioId ?? songInfo.songmid)
